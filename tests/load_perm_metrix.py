...
parser.add_argument('--iter_idx', type=int, default=4)
...
if __name__ == '__main__':
    with open(os.path.join(args.perm_metrix_file_dir, 'perm_matrices.pickle'), 'rb') as f:
        # (num_sample, 5, num_points, num_points)
        # num_sample 是测试集的大小
        # 5 是eval的迭代次数
    ...
    perm_metrix = perm_metrix_all[args.sample_idx][args.iter_idx]
    print('Selected perm metrix: sample idx={}, iter idx={}'.format(args.sample_idx, args.iter_idx))
    save_name_txt = os.path.join(args.perm_metrix_file_dir, 'perm_matrix_sample-{}_iter-{}.txt'.format(args.sample_idx, args.iter_idx))
    ...
    plt.colorbar()
    save_name_png = os.path.join(args.perm_metrix_file_dir, 'perm_matrix_sample-{}_iter-{}.png'.format(args.sample_idx, args.iter_idx))
