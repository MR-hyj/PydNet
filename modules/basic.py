    ...
    def reset_scale(self, input):
        ...
        if self.padding_mode == 'circular':
            ...
            self.scale = Parameter(
                (F.conv1d(ba, bw, self.bias, self.stride, self.padding, self.dilation, self.groups).std() \
                 / F.conv1d(torch.sign(ba), torch.sign(bw), self.bias, self.stride, self.padding, self.dilation,
                            self.groups).std()).float().to(ba.device))
...
class BiConv2d(torch.nn.Conv2d):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, stride=1, padding=0, dilation=1,
        ...
        super(BiConv2d, self).__init__(in_channels, out_channels, kernel_size, stride, padding,
...
class BiConv2dXNOR(torch.nn.Conv2d):
    ...
    def __init__(self, in_channels: int, out_channels: int, kernel_size: int, stride=1, padding=0, dilation=1,
                 groups=1, bias=True, padding_mode='zeros', device=None, dtype=None):
        super(BiConv2dXNOR, self).__init__(in_channels, out_channels, kernel_size, stride, padding,
                                           dilation, groups, bias, padding_mode, device, dtype)
...
class BinaryQuantize(Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input[input[0].gt(1)] = 0  # 大于1的部分为0
        grad_input[input[0].lt(-1)] = 0  # 小于-1的部分为0
        return grad_input
...
class BiLinearLSR(torch.nn.Linear):
    ...
    def reset_scale(self, input):
        ...
        bw = bw - bw.mean()
        self.scale = Parameter(
            (F.linear(ba, bw).std() / F.linear(torch.sign(ba), torch.sign(bw)).std()).float().to(ba.device))
...
class BiLinearIRNet(torch.nn.Linear):
    ...
    def forward(self, input):
        ...
        sw = torch.pow(torch.tensor([2] * bw.size(0)).cuda().float(),
                       (torch.log(bw.abs().mean(-1)) / math.log(2)).round().float()).view(-1, 1).detach()
        k, t = self.k, self.t
...
def BwMLP(channels, batch_norm=True, activation='ReLU', bilinear='BiLinear'):
    return Seq(*[
        Seq(biLinears[bilinear](channels[i - 1], channels[i], binary_act=False), activations[activation](),
            BN(channels[i]))
        for i in range(1, len(channels))
    ])
